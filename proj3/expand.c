 ...
 * index:012345678901
 *  name:123456789012
 *  name:#include?"?" shortest file is 12 char's
 * return:
...
int gen_is_include_line( char* l){
  ...
  if( NULL != strchr( l, '#')){ //attempted include statment
   if( strlen(l) >= 12 && l[0] == '#' && NULL != strstr(l, "#include") &&
          0 != isspace( (int)l[8])){
      temp = NULL;
      int counter,  times_found = 0, first_quote_flag = 0;
      for( counter = 8; counter < strlen( l)-1; ++counter){
        temp = l + counter;
        if( *temp == '\"') first_quote_flag = 1;
        if( first_quote_flag == 0 && isspace((int)*temp) == 0){
          fprintf( stderr, "INVALID INCLUDE, TERMINATING 2\n");
          abort();
        }
        else if( first_quote_flag == 1){
          temp = strchr( l+counter+1, '\"'); //for loop stops before '\0'
          if( temp == NULL){
            fprintf( stderr, "INVALID INCLUDE, TERMINATING 3\n");
            abort();
          }
          else return 1; //the second quote has been found
          // this is enough information to quit #include<space(s)>"text_ _"???
        }
      }
      return 0;
    }
    else{
      fprintf( stderr, "INVALID INCLUDE, TERMINATING 1\n");
      abort();
    }
  }
